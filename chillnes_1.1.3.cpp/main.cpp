...
Base b1(p1, base_size), b2(p2, base_size), b3(p3, base_size), b4(p4, base_size), b5(p5, base_size), b6(p6, base_size), b7(p7, base_size);
...
void Game::initBase() {
    for(int i = 0; i < bases.size(); i++) {
        bases[i].size = base_size;
        bases[i].picture.setPosition(bases[i].pos.get_x(), bases[i].pos.get_y());
        bases[i].picture.setSize(sf::Vector2(bases[i].size, bases[i].size));
        bases[i].picture.setFillColor(bases[i].color);
        bases[i].picture.setOrigin(bases[i].size / 2, bases[i].size / 2);
    }
}
...
void Game::initAnimal() {
    ...
    for(int i = 0; i < animals.size(); i++){
        if(position.distance(animals[i].get_pos()) < animal_size*2) nearby = true;

        // for(int i = 0; i < animals.size(); i++){
        //     if(position.distance(animals[i].get_pos()) < animal_size*2) nearby = true;
        }

        if (nearby == false and energy >= 5 and position.get_y() < height and position.get_x() < width) {
            energy -= 5;
            Point aim_ = Point(0, 0);
            auto beast = Simple_Animal(5, 100, 10, aim_, position);
